...
def get_ref_program(stride=1, padding=1, dilation=1, eps=1e-3):
    """Reference implementation: Conv2d + BatchNorm(training mode)."""

    def conv_bn_core(X_nchw, W_oihw, BN_Gamma, BN_Beta, BN_Mean, BN_Var):
        ...
        return (Y_nchw - mean) * torch.rsqrt(var + eps) * gamma + beta

    compiled_conv_bn_core = None
    if hasattr(torch, "compile"):
        try:
            compiled_conv_bn_core = torch.compile(conv_bn_core)
        except Exception:
            compiled_conv_bn_core = None

    def ref_program(X, W, BN_Gamma, BN_Beta, BN_Mean, BN_Var, Y):
        nonlocal compiled_conv_bn_core
        X_nchw = X.permute(0, 3, 1, 2)
        W_oihw = W.permute(3, 2, 0, 1)

        if compiled_conv_bn_core is not None:
            try:
                Y_nchw = compiled_conv_bn_core(X_nchw, W_oihw, BN_Gamma, BN_Beta, BN_Mean, BN_Var)
            except Exception:
                compiled_conv_bn_core = None
                Y_nchw = conv_bn_core(X_nchw, W_oihw, BN_Gamma, BN_Beta, BN_Mean, BN_Var)
        else:
            Y_nchw = conv_bn_core(X_nchw, W_oihw, BN_Gamma, BN_Beta, BN_Mean, BN_Var)
...
def _conv2d_bn_kernel_factory(N, C, H, W, F_out, KH, KW, S=1, P=1, D=1, eps=1e-3):
    """Return a Conv2d+BN kernel factory."""
    ...
    def kernel(block_M=None, block_N=None, block_K=None, num_stages=None, thread_num=None):
        ...
        def conv_bn_kernel(
            ...
            bn_gamma: T.Tensor((F_out,), out_dtype),
            bn_beta: T.Tensor((F_out,), out_dtype),
            bn_mean: T.Tensor((F_out,), out_dtype),
            bn_var: T.Tensor((F_out,), out_dtype),
            ...
            with T.Kernel(T.ceildiv(F_out, block_N), T.ceildiv(M_eff, block_M), threads=thread_num) as (bx, by):
                ...
                for i, j in T.Parallel(block_M, block_N):
                    ...
                    if m_idx < M_eff and n_idx < F_out:
                        x = out_shared[i, j]
                        gamma = bn_gamma[n_idx]
                        beta = bn_beta[n_idx]
                        mean = bn_mean[n_idx]
                        var = bn_var[n_idx]

                        inv_std = 1.0 / T.sqrt(var + T.cast(eps, out_dtype))
                        out_flat[m_idx, n_idx] = (x - mean) * inv_std * gamma + beta
...
def build_conv2d_bn_kernel(N, C, H, W, F_out, KH, KW, S=1, P=1, D=1, config=None):
    """Build a compiled Conv2d+BN kernel using cost-model selection or a given config."""
    if config is None:
        config, _ = select_best(DESCRIPTOR, N=N, C=C, H=H, W=W, OC=F_out,
                                KH=KH, KW=KW, stride=S, padding=P)
    factory = _conv2d_bn_kernel_factory(N, C, H, W, F_out, KH, KW, S, P, D)
    prim_func = factory(**config)
    return tilelang.compile(prim_func, target="auto")


def estimate_total_flops(N, OH, OW, F_out, C, KH, KW):
    """Estimate FLOPs including conv and fused epilogue (BN)."""
    conv_flops = 2 * N * OH * OW * F_out * C * KH * KW
    out_elems = N * OH * OW * F_out
    bn_flops = 6 * out_elems
...
def verify_correctness(kernel, ref_prog, N, C, H, W, F_out, KH, KW, S, P, D):
    ...
    # Compute per-channel batch stats from current conv output to emulate BN training forward.
    X_nchw = X.permute(0, 3, 1, 2)
    W_oihw = W_weight.permute(3, 2, 0, 1)
    Y_conv = F.conv2d(X_nchw, W_oihw, stride=S, padding=P, dilation=D).to(torch.float32)
    BN_Mean = Y_conv.mean(dim=(0, 2, 3)).to(torch.float16)
    BN_Var = Y_conv.var(dim=(0, 2, 3), unbiased=False).to(torch.float16)
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark Conv2d+BN fusion with autotuning")
    parser.add_argument("--output", default="conv2d_bn_results.csv", help="Output CSV path")
    ...
    print("\n" + "=" * 80)
    print("Operator: Conv2d + BatchNorm")
    ...
    for idx, shape in enumerate(shapes_to_test):
        ...
        try:
            best_config, _ = select_best(DESCRIPTOR, N=N, C=C, H=H, W=W, OC=F_out,
                                         KH=KH, KW=KW, stride=S, padding=P)
            kernel = build_conv2d_bn_kernel(N, C, H, W, F_out, KH, KW, S, P, D,
                                            config=best_config)
...
class Conv2dBnDescriptor(TileLangKernelBase):
    @property
    def name(self):
        return "conv2d_bn"
